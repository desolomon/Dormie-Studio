...
function makeWoodTexture(repeatX = 1, repeatY = 1): THREE.CanvasTexture {
...
function makeWallTexture(wallColor: string): THREE.CanvasTexture {
  const c = document.createElement("canvas");
  c.width = 256; c.height = 256;
  const ctx = c.getContext("2d")!;
...
export default function Room({ width, depth, height, themeColors: colors }: RoomProps) {
  const woodFloor  = useMemo(() => makeWoodTexture(5, 4), []);
  const wallTex    = useMemo(() => makeWallTexture(colors.wall), [colors.wall]);
  const comforterTex = useMemo(() => makeComforterTexture(colors.bedding), [colors.bedding]);
  const rugTex     = useMemo(() => makeRugTexture(colors.rug), [colors.rug]);
    ...
    <group>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0, 0]} receiveShadow>
        ...
        <meshStandardMaterial
          color={colors.floor}
          map={colors.floorIsMarble ? undefined : woodFloor}
      ...
      {/* Front wall — very transparent */}
      <mesh rotation={[0, Math.PI, 0]} position={[0, height / 2, hd]}>
        <planeGeometry args={[width, height]} />
        <meshStandardMaterial color={colors.wall} roughness={0.85} transparent opacity={0.08} />
      </mesh>
      ...
      />
      <Desk
        position={[-deskX, 0, deskZ]}
        dd={DD}
        colors={colors}
      />
      <WallShelf position={[-hw + 0.08, 5.8, shelfZ]} side="left" color={colors.bookshelf} />
      ...
      <Desk
        ...
        colors={colors}
      />
      <WallShelf position={[hw - 0.08, 5.8, shelfZ]} side="right" color={colors.bookshelf} />
...
// ─── Lofted bed ───────────────────────────────────────────────────────────────
function LoftedBed({ position, bl, bw, colors, comforterTex }: {
  position: [number, number, number];
  bl: number; bw: number;
  colors: ThemeColors;
  comforterTex: THREE.CanvasTexture;
}) {
  const LOFT = 2.6;
  const SLATS = 7;
  const DRW = bw * 0.75;
    ...
    <group position={position}>
      ...
      {([-bw / 2 + 0.1, bw / 2 - 0.1] as number[]).flatMap((x) =>
        ...
        <mesh key={i} position={[x, LOFT / 2, z]} castShadow>
          ...
          <meshStandardMaterial color={colors.bedFrame} roughness={0.5} />
        </mesh>
      ))}
      ...
      {Array.from({ length: SLATS }).map((_, i) => {
        ...
        return (
          <mesh key={i} position={[0, LOFT - 0.28, z]} castShadow>
            ...
            <meshStandardMaterial color={colors.bedFrame} roughness={0.6} />
          </mesh>
        );
      })}
      ...
      <mesh position={[0, LOFT + 0.78, 0]} castShadow>
        ...
        <meshStandardMaterial color={colors.bedding} map={comforterTex} roughness={0.92} />
      </mesh>

      {/* Pillows — alternating pillow1 / pillow2 */}
      {[bw * 0.18, -bw * 0.18].map((px, i) => (
        <mesh key={i} position={[px, LOFT + 0.9, -bl / 2 + 0.55]} castShadow>
          <boxGeometry args={[bw * 0.38, 0.14, 0.54]} />
          <meshStandardMaterial color={i === 0 ? colors.pillow1 : colors.pillow2} roughness={0.9} />
        </mesh>
      ))}
...
// ─── Desk + chair + lamp ──────────────────────────────────────────────────────
function Desk({ position, dd, colors }: {
  ...
  return (
    <group position={position}>
      ...
      <mesh position={[0, 2.5, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color={colors.desk} roughness={0.4} />
      </mesh>
      {/* Legs */}
      ...
      <TaskChair position={[0, 0, dd / 2 + 1.0]} color={colors.chair} />
      {/* Lamp (always shown with accent color) */}
      <DeskLamp position={[DW / 2 - 0.5, 2.55, -dd / 2 + 0.32]} color={colors.accent} />
    </group>
  );
}
...
// ─── Wall shelf (bookcase) ────────────────────────────────────────────────────
function WallShelf({ position, side, color }: {
  position: [number, number, number]; side: "left" | "right"; color: string;
}) {
  ...
  return (
    <group position={position}>
      ...
      <mesh position={[0, 0, 0.07]}>
        ...
        <meshStandardMaterial
          color="#b5d5ee" transparent opacity={0.22} roughness={0} metalness={0.1}
